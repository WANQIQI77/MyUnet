...
def Compute_mIoU(gt_dir, pred_dir, png_name_list, num_classes, name_classes):
    ...
    gt_np = [join(gt_dir, x + ".npy") for x in png_name_list]
    #
    for ind in range(len(pred_imgs)):
        pred = np.array(Image.open(pred_imgs[ind]))
        # label = np.array(Image.open(gt_np[ind]))
        label = np.load(gt_np[ind])

        if len(label.flatten()) != len(pred.flatten()):
            print(
                'Skipping: len(gt) = {:d}, len(pred) = {:d}, {:s}, {:s}'.format(
                    len(label.flatten()), len(pred.flatten()), gt_np[ind],
                    pred_imgs[ind]))
            continue
        ...
        if ind > 0 and ind % 10 == 0:
            print('{:d} / {:d}: mIou-{:0.2f}%; mAP-{:0.2f}%; Accuracy-{:0.2f}%'.format(
                ind,
                len(gt_np),
    ...
    print('===> mIoU: ' + str(round(np.nanmean(IoUs) * 100, 2)) + '; mAP: ' + str(
...
def show_results(miou_out_path, hist, IoUs, PA_Recall, Precision, name_classes, tick_font_size=12):
    ...
    draw_plot_func(PA_Recall, name_classes, "mAP = {0:.2f}%".format(np.nanmean(PA_Recall) * 100), "Pixel Accuracy", \
                   os.path.join(miou_out_path, "mAP.png"), tick_font_size=tick_font_size, plt_show=False)
    print("Save mAP out to " + os.path.join(miou_out_path, "mAP.png"))
